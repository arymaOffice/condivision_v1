...
if(isset($newid)) { $id = (is_numeric($newid)) ? check($newid) : exit; $where = "WHERE id = $id ";}	
	...
	while ($riga = mysql_fetch_array($risultato)) 
	{
		...
		if($id == 1 && in_array($chiave,$campi_today_time)) $valore = date('Y-m-d H:i');
		...
		if(is_to_use($chiave) == 1){
			...
			if(isset($$chiave) && $id == 1 && !is_array($$chiave) && $chiave != 'label') $valore = $$chiave; 
			$SEL_val = 'SEL_'.$chiave;

			if(isset($$SEL_val) && $$SEL_val > -1 && $id == 1 && !is_array($$SEL_val)) $valore = $$SEL_val; 
			if($chiave == "status_potential" && isset($_GET['status_potential']) && $id == 1 && check($_GET['status_potential']) > 0) $valore = check($_GET['status_potential']);  
			if($chiave == "status_potential" && isset($_GET['status_potential']) && $id == 1 && check($_GET['status_potential']) < 0) $valore = 0;  
		    ...
		    if($chiave == "codice_fiscale" || $chiave == "codice_fiscale_legale"){ $extra = '<a href="http://www.codicefiscaleonline.com/" target="_blank">Genera</a> | <a href="https://telematici.agenziaentrate.gov.it/VerificaCF/Scegli.do?parameter=verificaCf" target="new">Verifica</a>'; }
			...
			if(!in_array($chiave,$preselezionaPrimaOption)) echo "<option value=\"-1\">Seleziona...</option>\r\n";
			...
			case 13:		// Input Text	
			$disabled = ($valore == 'NO' || $valore == '') ? 'disabled' : '';
			$checked = ($valore == 'NO' || $valore == '') ? '' : 'checked';
  		    echo "<p class=\"input_text\"><label for=\"$chiave\">".record_label($chiave,CONNECT,1)."</label>
  		    <input $checked type=\"checkbox\" data-control=\"$chiave\" class=\"EnableDisable\" style=\"display: inline; width: auto; margin: 0; border: 1px solid gray;\">
			...
			$selected = (in_array($key,$valore)) ? "checked=\"checked\"" : "";	
		   	...
		   	if($key > 0) echo "<input name=\"$chiave"."[".$key."]"."\" type=\"checkbox\" id=\"".$chiave.$key."\" value=\"".$key."\" $selected /><label for=\"".$chiave.$key."\">".$label."</label>";
			
			}
			
			if(!isset($$chiave) || !is_array($$chiave)) { echo "ATTENZIONE: ".$chiave." non trovata o non e un array.";  }	
			echo '</span></p>';
			if($chiave == 'mesi_di_interesse') echo '<a href="javascript:return false;"  class="msg orange showCalDisp" ><i class="fa fa-calendar-times-o" aria-hidden="true"></i> Calendario Disponibilità</a>';
