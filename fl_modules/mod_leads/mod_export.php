...
$insert = "insert into fl_esportazioni_excel (query,account_id) VALUES ('select  DATE(NOW()),CURTIME(),

modello_interesse,pot.id as poteID,nome,cognome,indirizzo,email,telefono,job_title,marca,modello,anno_immatricolazione,chilometri_percorsi,alimentazione,targa,note,ca.oggetto as caog,test_drive,partita_iva,ragione_sociale,priorita_contatto,permuta,pagamento_vettura

FROM fl_potentials as pot LEFT JOIN fl_campagne_attivita ca ON ca.id = pot.campagna_id LEFT JOIN fl_veicoli v ON v.parent_id = pot.id WHERE ".$tipologia_main." ','".$_SESSION['number']."')";
mysql_query($insert);
mysql_close(CONNECT);
exit;
...
// $leads = GQS('fl_potentials as pot LEFT JOIN fl_campagne_attivita ca ON ca.id = pot.campagna_id LEFT JOIN fl_veicoli v ON v.parent_id = pot.id ', 'DATE(NOW()),CURTIME(),

// modello_interesse,poteID,nome,cognome,indirizzo,email,telefono,job_title,marca,modello,anno_immatricolazione,chilometri_percorsi,alimentazione,targa,note,caog,test_drive,partita_iva,ragione_sociale,priorita_contatto,permuta,pagamento_vettura

// ,pot.id as poteID,ca.oggetto as caog', $tipologia_main);
