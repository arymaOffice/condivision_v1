...
$email  = convert(check($_POST['email']));	
$email2  = convert(check($_POST['email2']));
$user  = implode("",explode(" ",convert(check($_POST['user']))));		  
...
if(smail($mail_to,$mail_subject,$mail_body,'') ) {
header("Location: ./?esito=Nuovo Utente Creato e Mail inviata correttamente!&indietro=1"); 
exit;

} else {
header("Location: ./?esito=Nuovo Utente Creato e Recapito e-Mail fallito!&indietro=0"); 
exit; 
}				 
