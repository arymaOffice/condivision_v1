...
VALUES (NULL, '1', '0', '$location_testdrive', '$veicolo', '$start_meeting', '$end_meeting' , '$lead_id', '$nome $cognome', '', '$operatore', '$operatore', '0', NOW(), NOW());";
    ...
    public function insert_lead_app()
    {
        ...
        $id_alimentazione = array('NULL' => 0, 'D' => 1, 'B' => 2, 'G' => 3, 'M' => 4, 'I' => 5, 'E' => 6);

        //$alimentazione = $vettura_posseduta_alimentazione = array('Sconosciuta','Diesel','Benzina','GPL','Metano','Ibrida','Elettrica');
        ...
        $attivita_info = GQD('fl_campagne_attivita', 'campagna_id', 'id = ' . @$source_potential);
        $campagna_id = $attivita_info['campagna_id'];
...
// INSERIMENTO VEICOLO
            if (trim($marca . $modello) != '') {
                ...
                $modello = (isset($modello[0]) && $modello[0] != '') ? $modello[0] : '';
                $sql = "INSERT INTO `fl_veicoli` (`id`, `workflow_id`, `parent_id`, `tipologia_veicolo`, `data_acquisto`, `pagamento_veicolo`, `marca`, `modello`, `colore`, `descrizione`, `anno_immatricolazione`, `chilometri_percorsi`, `alimentazione`, `targa`, `quotazione_attuale`, `data_creazione`, `data_aggiornamento`, `operatore`)
VALUES (NULL, '16', '$lead_id', '$tipologia_veicolo', '$data_acquisto', '$pagamento_veicolo', '$marca', '$modello', '$colore', '$descrizione', '$anno_immatricolazione', '$chilometri_percorsi', '" . $id_alimentazione[$alimentazione] . "', '$targa', '$quotazione_attuale', NOW(), NOW(), '$operatore');";
                if (mysql_query($sql, CONNECT)) {$mail_message .= "<p>Veicolo Inserito!</p>";}
            }
    ...
    public function get_attivita()
    {
        ...
        $query = "SELECT * FROM `fl_campagne_attivita`  WHERE data_inizio < NOW() AND data_fine > NOW() and id !=  1 and id !=  25 ORDER BY data_creazione DESC";
...
/*Funzioni Globali e Utility */
    ...
    public function get_data_lead($campi = '',$user_id = '',$when = '')
    {
        ...
        foreach($campi as $key => $value){
            if(!in_array($value,$campi_controllo)) {
                $this->contenuto['class'] = 'red';
                $this->contenuto['esito'] = "Error field not allowed.";
                $this->cnv_makedata();
            }
            if($value == 'id'){ $campi[$key] = 'pot.id as POTID';}
        }
