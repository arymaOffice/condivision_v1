...
if (isset($_GET['workflow_id'])) {
    $module_title .= ' ' . @$workflow_id[check($_GET['workflow_id'])];
}
...
foreach ($campi as $chiave => $valore) {
    if (in_array($chiave, $basic_filters)) {
        ...
        if (select_type($chiave) != 9 && select_type($chiave) != 19 && select_type($chiave) != 5 && select_type($chiave) != 2 && $chiave != 'id') {$cont = (isset($_GET[$chiave])) ? check($_GET[$chiave]) : '';
            echo '<label>' . $valore . '</label><input type="text" name="' . $chiave . '" value="' . $cont . '" placeholder="' . $valore . '" />';}
...
while ($riga = mysql_fetch_array($risultato)) {
    ...
    echo "<td><strong id=\"chilometri_percorsi" . $riga['id'] . "\">" . ($riga['chilometri_percorsi']+@$riga['chilometri_rilevati']) . '</strong> KM - ' . @mydate($riga['anno_immatricolazione']) . "</span><br>" . $riga['telaio'] . "</td>";
    echo "<td>$titolare</td>";
    echo "<td>" . mydate($riga['data_acquisto']) . "<br><br> <span class=\"msg green\">" . @$pagamento_veicolo[$riga['pagamento_veicolo']] . "</span></td>";
    echo "<td>" . @$riga['targa'] . "</td>";

    $msg_quotazione = '';
    if ($riga['data_quotazione'] != '0000-00-00') {$msg_quotazione = $riga['dataQuotazione'];}

    echo '<td><input type="text"  id="quotazione' . $riga['id'] . '" style="width:80%;" value="' . @$riga['quotazione_attuale'] . '" name="quotazione_attuale" class="updateField"  data-rel="' . $riga['id'] . '" /> &euro; </td>';
    echo "<td>" . $msg_quotazione . "</td>";
    ...
    if ($riga['vdi'] != '') {
        ...
        echo "
			...
			<a href=\"mod_opera.php?id=" . $riga['id'] . "&q=km&vdi=" . $riga['vdi'] . "\"  class=\"msg green ajaxLoad\"  >Rileva Chilometri</a> <br><br>";

    }

    echo " <a href=\"mod_versioni.php?id=" . $riga['id'] . "&eurotax&targa=" . @$riga['targa'] . "\"  class=\"msg ajaxLoad\" id='btquotazione" . $riga['id'] . "'><span class='msg orange '>Quot. EUROTAX</span></a>
			";

    echo "<a href=\"mod_opera.php?id=" . $riga['id'] . "&cestinaQuotazione\"  class=\"ajaxLoad\" ><span class='msg orange '>Cancella quotazione</span></a>";
    echo " <a href=\"mod_opera.php?id=" . $riga['id'] . "&eurotax&targa=" . @$riga['targa'] . "\"  class=\"msg ajaxLoad\" ><span class='msg orange '>scegli versione</span></a></td>";
    ...
    if ($_SESSION['usertype'] == 0) {
        echo "<a href=\"../mod_basic/action_elimina.php?gtx=$tab_id&amp;unset=" . $riga['id'] . "\" title=\"Cancella\"  onclick=\"return conferma_del();\"><i class=\"fa fa-trash-o\"></i></a>";
    }
