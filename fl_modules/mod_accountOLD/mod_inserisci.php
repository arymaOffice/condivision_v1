...
$class = (isset($_GET['success'])) ? 'green' : 'red'; echo '<p class="esito '.$class.'">'.check($_GET['esito']).'</p><p style="text-align: center;">

<a title="Torna Indietro" href="javascript:history.back();">&lt;&lt;Indietro</a></p>'; }  else { ?>
...
foreach($tipo as $valores => $label){ // Recursione Indici di Categoria
			$selected = (isset($_GET['anagrafica_id']) && $valores == 2) ? 'selected="selected" ' : '' ;
			echo "<option value=\"$valores\" $selected>".ucfirst($label)."</option>\r\n";
			} ?>
...
<input name="user" type="text"  size="40" maxlength="255" value="<?php if(isset($_GET['email'])) echo substr(str_replace('@','',str_replace('.',date('Y'),check($_GET['email']))),0,15); ?>"  />
</p></div>
...
<div class="form_row">
<p class="input_text">
<input name="auto_pass" type="checkbox" id="auto_pass" checked="checked" /><label for="auto_pass" onclick="spegni();">Crea automaticamente la password</label> 
</p>
