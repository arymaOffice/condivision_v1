...
if(isset($_POST['doc_acquisto_id'])) {
	...
	inserisci_voci_acquisto($doc_acquisto_id,$codice,$descrizione,$imponibile,$quantita,$valuta,$aliquota);
	...
	*/
	...
	foreach($_POST['id'] as $key ) {
		...
		if($_POST['fornitore'.$key] != $primo_fornitore ){
			...
			$numero_doc = GQD('fl_doc_acquisto','(numero_documento + 1) as n_doc',' anno_di_competenza = YEAR(NOW()) ORDER BY numero_documento DESC LIMIT 1');

			$numero_doc['n_doc'] = ($numero_doc['n_doc'] == '') ? 1 : $numero_doc['n_doc'];


			//inserimento docuemnto di acquisto
			$insert_doc_acq = "INSERT INTO `fl_doc_acquisto` ( `anno_di_competenza`, `tipo_doc_acquisto`, `anagrafica_id`, `ragione_sociale`,`data_documento`, `numero_documento`, `oggetto_documento`,`data_creazione`, `data_aggiornamento`, `operatore`) VALUES (NOW(),4,'".$anagrafica_id."','',NOW(),'".$numero_doc['n_doc']."' ,'".$oggetto."',NOW(),NOW(),'".$_SESSION['number']."');";
		...
		$insert_doc_voci = "INSERT INTO `fl_doc_acquisto_voci`( `parent_id`,  `codice`, `descrizione`,`note`, `unita_di_misura`, `quantita`, `valuta`, `importo`, `data_creazione`, `operatore`) VALUES ('".$parent_id."','".$info_materia['codice_articolo']."','".$info_materia['descrizione']."','".$note."','".$info_materia['unita_di_misura']."','".$quantita."','EUR','".$info_materia['ultimo_prezzo']."',NOW(),'".$_SESSION['number']."')";

		$insert_doc_voci = mysql_query($insert_doc_voci,CONNECT);
		
		$fabbisogno_id = filter_var($_POST['fabb'.$key],FILTER_SANITIZE_NUMBER_INT);

		$update = "UPDATE fl_ricettario_fabbisogno set ordine_id = ".$parent_id." WHERE id = ".$fabbisogno_id;
