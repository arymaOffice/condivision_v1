...
if (isset($_POST['inviaSMS'])) 
{
  ...
  foreach($_POST['destinatario'] as $dest_id){ 
	...
	$messaggio = str_replace('[cry_lead_id]',base64_encode($destinatario['id']).' ',$messaggio);
  	...
  	if(sms($to,$messaggio,$from))  { 
		
		$send++; 
		$saveSMS = "INSERT INTO `fl_sms` (`id`, `status`, `template`, `from`, `to`, `body`, `data_ricezione`, `data_creazione`) VALUES (NULL, '0', '$template', '$from', '$to', '$messaggio', '', NOW());"; // Registra SMS inviato
