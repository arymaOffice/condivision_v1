...
// Le ricordiamo che pu&oacute; accedere all'area riservata da qui: <a href=\"https:" . ROOT . $cp_admin . "\" title\"Area Riservata\">https:" . ROOT . $cp_admin . "</a>.<br /> </p>
...
//             $mail_body = str_replace("{{oggetto}}", $mail_subject, $mail_body);
...
if (isset($_POST['modifica_pass'])) {
    ...
    if ($pass1 === $pass2) {
        if (!preg_match('/^(?=.*\d)(?=.*[A-Za-z])[0-9A-Za-z!@#$%]{8,16}$/', $pass1)) {
            mysql_close(CONNECT);
            header("Location: " . $rct . "&id=$id&esito=" . urlencode('La password deve contenere valori alfanumerici o !@#$% e deve essere di lunghezza tra 8 e 16 caratteri&menu=1'));
            exit();
        }
        ...
        if (mysql_affected_rows() == 0) {
            ...
            $queryx = "UPDATE `fl_account` SET `password`= '$password',`ip`='$ip',`data_aggiornamento`=NOW(),`operatore`=" . $_SESSION['number'] . ",`aggiornamento_password`=NOW() WHERE id = $id LIMIT 1";
...
//echo $inviopass." -------".$queryx; exit;
                // intestazioni addizionali
                $mail_to = $riga['email'];
                $mail_subject = "Modifica Password per $user su " . str_replace('//','',ROOT);
                $mail_message = "<p>La tua password è stata modificata con successo! <br />
								Se non sei stato tu a modificare la password, è possibile che il tuo acccount sia stato violato.<br>
                ...
                if (smail($mail_to, $mail_subject, $mail_body)) {
                    if ($_SESSION['aggiornamento_password'] < -80) {
                        ...
                        header("Location: " . $rct . "&id=$id&success&esito=Password aggiornata e conferma via email!&indietro=1");
                    }
                    exit;
...
if (isset($_POST['account'])) {
    ...
    $external_id = (isset($_POST['external_id']) && check($_POST['external_id']) > 1) ? check($_POST['external_id']) : 'NULL';
...
//header("Location: ./?esito=Inserire una email valida.&menu=1");
    ...
    if (isset($_POST['user'])) {
        if (!preg_match('/^[A-Za-z0-9]{6,20}$/', $user)) {
//header("Location: ./?esito=Username Non valido. Deve contenere valori alfanumerici e deve essere di lunghezza tra 6 e 20 caratteri&menu=1).");
            echo json_encode(array('action' => 'info', 'class' => 'red', 'url' => '', 'esito' => "Username Non valido. Deve contenere valori alfanumerici e deve essere di lunghezza tra 6 e 20 caratteri"));
            mysql_close(CONNECT);
            exit;
        }
    }
...
//header("Location: ./?esito=La password deve contenere valori alfanumerici e deve essere di lunghezza tra 8 e 15 caratteri&menu=1).");
            echo json_encode(array('action' => 'info', 'class' => 'red', 'url' => '', 'esito' => urlencode('La password deve contenere valori alfanumerici o !@#$% e deve essere di lunghezza tra 8 e 16 caratteri')));
            mysql_close(CONNECT);
            exit;
        }
...
// Funzione
    if ($nominativo != "" && $email != "") {
        ...
        if (mysql_affected_rows() > 0) {
//header("Location: ./?esito=Esiste un utente con questa mail o username.&menu=1).");
            echo json_encode(array('action' => 'info', 'class' => 'red', 'url' => '', 'esito' => "Esiste un utente con questa mail o username"));
            mysql_close(CONNECT);
            exit;
        }

        $sql = "INSERT INTO `fl_account` (`id` ,`external_id`,`cuid`,`uid`,`motivo_sospensione`,`attivo` ,`anagrafica` ,`marchio` ,`tipo` , `persona_id` ,`email` ,`nominativo` ,`ip_accesso` ,`user` ,`password` ,`foto` ,`visite` ,`ip` ,`data_creazione`,`data_scadenza`,`data_aggiornamento`,`operatore`, `aggiornamento_password`)
					 VALUES ( $external_id ,$external_id,1,1, '','$attivo', '$anagrafica', '$marchio', '$tipo','$persona_id','$email','$nominativo','$ip_accesso', '$user', '$pass','$foto', '0', '$ip', NOW(), '2050-12-31', NOW(), '" . $_SESSION['number'] . "',  NOW() );";
...
<p>Puoi accedere al gestionale da qui: <a href=\"https:" . ROOT . $cp_admin . "\" title\"Area Riservata\">https:" . ROOT . $cp_admin . "</a>.<br /> </p>
...
<p>&nbsp;</p>";
            ...
            $mail_body = str_replace("{{oggetto}}", $mail_subject, $mail_body);
            $mail_body2 = str_replace("{{oggetto}}", $mail_subject, $mail_body2);
...
// Mancato inserimento Campi
        ...
        header("Location: ./?esito=Inserire i campi obbligatori.&menu=1");
        exit;
    } //chiudi funzione
...
if (isset($_GET['user_id']) && $_SESSION['usertype'] == 0) {
    ...
    $profilod = $userid;
    if (mysql_query($profilo, CONNECT)) {
        $profilod = "Creazione profilo utente avvenuta.";
    }
