...
<?php if (defined('qualificazioneLeads') && $status_potential_id != 4) {
...
<div id="filtri" class="filtri">
  ...
  <form method="get" action="" id="fm_filtri">
    ...
    <input type="radio" name="qualificati" id="qualificati1" value="1" <?php if ($qualificati_id == 1) {
    echo 'checked';
}
    ?>>
    <label for="qualificati1">SI</label>

    <input type="radio" name="qualificati" id="qualificati0" value="0"  <?php if ($qualificati_id == 0) {
        echo 'checked';
    }
    ?>>
    <label for="qualificati0">NO</label>

    <input type="radio" name="qualificati" id="qualificati2" value="2"  <?php if ($qualificati_id == 2) {
        echo 'checked';
    }
    ?>>
    <label for="qualificati2">SUPER</label>

    <input type="radio" name="qualificati" id="qualificati3" value="-1"  <?php if ($qualificati_id == -1) {
        echo 'checked';
    }
    ?>>
...
while ($riga = mysql_fetch_array($risultato)) {
    ...
    if ($riga['data_scadenza'] != '0000-00-00' && $riga['data_scadenza'] != '') {
        ...
        if ($giorniCount == 0) {
            $giorni = '<span class="msg red">SCADE OGGI</span>';
        }

        if ($giorniCount == 1) {
            $giorni = '<span class="msg orange">Domani</span>';
        }
    ...
    if ($riga['data_scadenza_venditore'] != '0000-00-00' && $riga['data_scadenza_venditore'] != '') {
        ...
        if ($giorniCount == 0) {
            $giorni2 = '<span class="msg red">SCADE OGGI</span>';
        }

        if ($giorniCount == 1) {
            $giorni2 = '<span class="msg orange">Domani</span>';
        }
    ...
    $note = '';
    if (defined('MOSTRA_NOTE_LEADS_LISTA')) {
        $note = "<br> " . strip_tags(converti_txt($riga['note']));
    }

    $phone = phone_format($riga['telefono'], '39');
    $website = (isset($riga['website'])) ? www($riga['website']) : '';
    ...
    $locationInfo = (isset($riga['location_evento'])) ? $location_evento[$riga['location_evento']] : '';

    echo "<tr><td $colore><span class=\"Gletter\"></span></td>";

    echo "<td class=\"checkItemTd\">$input</td>";
    echo "<td><h3>$data_principale $feedback</h3>
			<span class=\"msg blue\" title=\"$locationInfo\">" . @$status_potential[$riga['status_potential']] . "</span> " . @$proprietario[@$riga['proprietario']] . "
			</td>";
    ...
    echo "<td>
			...
			$qualified " . $riga['ragione_sociale'] . " <strong title=\"" . strip_tags(converti_txt($riga['messaggio'])) . "\">" . ucfirst(checkValue($riga['nome'])) . " " . ucfirst(checkValue($riga['cognome'])) . "</strong></a> $synapsy $note </h2>
    ...
    echo "<td>";
    if (defined('tab_prefix') && @tab_prefix == 'hrc') {
        echo "<span class=\"msg orange\">" . @$anno_di_interesse[@$riga['anno_di_interesse']] . "</span>";
    }

    echo @$tipo_interesse[@$riga['tipo_interesse']] . "  " . $interesse;
    if (defined('tab_prefix') && @tab_prefix == 'hrc') {
        echo ' (' . $riga['numero_persone'] . " pax + " . $riga['numero_bambini'] . " B) ";
    }
    ...
    if ($riga['is_customer'] > 1) {
        echo "<td><a href=\"$new_contract\"><i class=\"fa fa-user $color_contract\" ></i></a></td>";
    }
