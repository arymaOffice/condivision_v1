...
foreach($_POST['codice'] as $item => $val) {
  ...
  //  echo '<br>'.$_POST['iva'][$item];
    ...
    $unita_di_misura = (isset($_POST['unita_di_misura'][$item])) ? check($_POST['unita_di_misura'][$item]) : '';
    ...
    if(isset($_POST['download'])) $qty = '-'.str_replace('-', '',$qty);

    if(trim($codice_fornitore.$descrizione) != '') {
    
    if($doc_vendita_id > 1){ // SOLO SE LEGATO AD UN DOCUMENTO DI ACQUISTO, aggiungo voci per contabilita
    ...
    if($id > 1) {  //AGGIORNO GIACENZA
        ...
        $updateGiacenza = "UPDATE ".$tables[116]." SET giacenza = giacenza +$qty $codiceUPDATE WHERE id = $id LIMIT 1";
        ...
        $idM = 0; // Creo solo un formato con materia = 0, poi tutti quelli a zero saranno associati a mano ad una materia 
...
//$referer = ($doc_vendita_id > 1) ? '../mod_doc_acquisto/mod_inserisci.php?id='.$doc_vendita_id : $_SERVER['HTTP_REFERER'];
