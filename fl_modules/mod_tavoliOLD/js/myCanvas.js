...
$(document).ready(function () {
	...
	$.get('mod_opera.php', { eventId: evento_id, ambiente_id: ambiente_id }, function (data) {
		...
		if (localStorage.ciSono != '0') { // se l'evento è gia settato recupero i tavoli
			...
			for (var i = 0; i < parseInt(localStorage.ciSono); i++) {
				...
				createTableOneP(parsed.idTavoli[i].numero_tavolo); //funzione che accetta un solo parametro
			}//end for
	...
	*/
	...
	function createTable(marginTop, marginLeft, name, textInsert, fila, type, categoria, numero) {
		...
		$.get('mod_opera.php', {
			insertTableId: name, insertEventId: evento_id, text: escape(textInsert),
			x: marginLeft, y: marginTop, type: type, categoria: categoria, numero: numero, diverso: false
		}, function (data) {
