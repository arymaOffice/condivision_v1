...
if(isset($newid)) { $id = (is_numeric($newid)) ? check($newid) : exit; $where = "WHERE id = $id ";}	
	...
	$campi_today = array('data_contatto','data_acquisto_vettura','data_test_drive','data_fattura','data_apertura','data_avvio','data_corrispettivo','data_versamento','data_scadenza_pec','data_sottoscrizione','data_inizio','data_fine','data_rinnovo','data_documento','data_operazione','data_scadenza');
	...
	while ($riga = mysql_fetch_array($risultato)) 
	{
		...
		if(isset($tabs_div)) { if(in_array($chiave,$tabs_div) && !is_numeric($chiave)) echo '</div><div id="tb_'.$chiave.'">'; }
		...
		if(is_to_use($chiave) == 1){
			...
			if(($chiave == "data_creazione" || $chiave == "data_assegnazione") && $id == 1) $valore = date('Y-m-d');
			if($chiave == "data_scadenza" && $id == 1) { $date = strtotime("+7 day"); $valore = date('d/m/Y',$date); }
			if(($chiave == "start_date" || $chiave == "end_date") && $id == 1) { $valore = date('d/m/Y H:i'); }
			if($chiave == "venditore" && $id == 1 && $_SESSION['usertype'] == 4) $valore = $_SESSION['number']; 
			if($chiave == "proprietario" && $id == 1 && $tabella != 'fl_potentials') $valore = $_SESSION['number'];
			...
			if($chiave == "anagrafica_id" && isset($_GET['ANiD']) && $id == 1) $valore =  (is_numeric($_GET['ANiD'])) ? check($_GET['ANiD']) : base64_decode(check($_GET['ANiD'])); //
			if($chiave == "potential_id" && isset($_GET['POiD']) && $id == 1)  $valore =  (is_numeric($_GET['POiD'])) ? check($_GET['POiD']) : base64_decode(check($_GET['POiD'])); //
			if($chiave == "account_id" && $id == 1) $valore = $_SESSION['number']; // Account id delle risorse
			...
			if($chiave == "menu_link" && isset($_GET['postback']) && $id == 1) $valore = str_replace('#','',$_SESSION['POST_BACK_PAGE']); 
			...
			<select class=\"$obbligatorio\" name=\"$chiave\" id=\"$chiave\" $disabled $onChange>\r\n";
			...
			if($chiave == 'gruppo_id') { echo "<p style=\"text-align: right;\">
			<a href=\"../mod_gruppi/mod_inserisci.php?id=1&auto=".$id."\" class=\"button\"> Crea Gruppo </a></p>";  }	
			break;
			...
			echo "<p class=\"input_text\"><textarea  class=\"mceEditor\" name=\"$chiave\" id=\"$chiave\" rows=\"20\" cols=\"100\" $disabled onkeyup=\"$('#info$chiave').html(this.value.length+' caratteri');\">".$valore."</textarea><span id=\"info$chiave\"></span></p>\r\n";
			break;
			...
			<label for=\"".$chiave.$valores."\" style=\"display: block;\">".$label."</label>";
			}
			if(!isset($$chiave) || !is_array($$chiave)) { echo "<p> ATTENZIONE: ".$chiave." non trovata o non e un array.</p>";  }	
			echo "</p>";
			break;
			...
			case 8:
			echo "<p class=\"pbox\"><label class=\"labelbox\">".record_label($chiave,CONNECT,1)."</label><span class=\"checkbox_container\">";
			...
			echo "<option value=\"Caricamento...\">Seleziona precedente...</option>\r\n";
			...
			if($valore != "") {
			echo '<div class="form_row" id="box_'.$chiave.'">';
  		    echo "<p class=\"input_text\"><label for=\"$chiave\">".record_label($chiave,CONNECT,1)."</label><input type=\"file\" name=\"$chiave\" id=\"$chiave\" size=\"5\" />".$valore."<input type=\"hidden\" name=\"old_file\" id=\"old_file\" value=\"$valore\" /><input type=\"checkbox\" name=\"del_file\" id=\"del_file\" style=\"display: inline; width: auto;\" value=\"1\" /> [X]</p>\r\n";
			echo '</div>';			
			}
			break;
			...
			$data = explode("-",$valore);		
			$valore_conv = str_replace('//','',@$data[2].'/'.@$data[1].'/'.@$data[0]);
	...
	foreach($tabs_ext as $chiave => $valore) { 
     ...
     if(!is_numeric($chiave)) echo "</div><div id=\"$chiave\"><iframe id=\"".sha1($chiave)."\"  style=\"width: 100%; border: none; height: 600px; \" src=\"".$contenutoFrame."\"></iframe>";
	}
    }
	...
	echo '</div></div>'; echo '<p class="savetabs"><a href="#" class="button salva" onclick="scheda.submit();">Salva</a></p>';
	} else { echo '</div></div><p class="savetabs"><a href="#" class="button salva" onclick="scheda.submit();">Salva</a></p>'; }
	}
	?>