...
if (isset($_GET['updateTable'])) { //update coords tavolo
    ...
    $zoomY = check($_GET['zoomY']);
       ...
       $updateCampo = "UPDATE $tabella_su_cui_operare SET asse_x = '$x',asse_y = '$y',angolare = '$angle' ,zoomX = '$zoomX',zoomY = '$zoomY'   WHERE id = $idTable" ;
...
if (isset($_GET['evento_id'])) { //in base all'ebento recupero il numero di tavoli
    ...
    if ($_SERVER['HTTP_HOST'] == 'calderonimartini.condivision.cloud') {
        $condition = 'ambienti = ' . $ambiente_id . '  AND id = ' . $evento_principale;
    }
    ...
    if ($evento_hrc[0]['multievento'] == 1) {
        ...
        if ($_SERVER['HTTP_HOST'] == 'calderonimartini.condivision.cloud') {
            $condition_1 = ' ambienti = ' . $ambiente_id;
        }
