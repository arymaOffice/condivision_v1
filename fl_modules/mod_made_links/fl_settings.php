...
if (!isset($_SESSION['anagrafica'])) {
    $_SESSION['anagrafica'] = 1;
}

if (isset($_GET['ANiD']) && $_SESSION['usertype'] < 2) {
    $_SESSION['anagrafica'] = check($_GET['ANiD']);
}
...
if (isset($_GET['potential_id'])) {
    $tipologia_main .= ' AND potential_id = ' . check($_GET['potential_id']) . ' ';
}
...
$categoria_link = $data_set->data_retriever($tables[135], 'descrizione', "WHERE id != 1 AND attivo = 1"); //Crea un array con i valori X2 della tabella X1
...
function select_type($who)
{
    ...
    $select = array('categoria_link');
    $disabled = array();
    $hidden = array('operatore', 'data_creazione', 'data_aggiornamento');
    $radio = array('attivo', 'multimonitor');
    $selectbtn = array('marchio');
