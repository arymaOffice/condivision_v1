...
if(!isset($_GET['periodo_inizio']) || !isset($_GET['periodo_fine']) || !isset($_GET['numero_documento'])) {

	echo "Devi specificare un periodo di inizio e fine e un numero documento iniziale";
	exit;
}
...
$periodo_inizio = convert_data($_GET['periodo_inizio'],1);
$periodo_fine = convert_data($_GET['periodo_fine'],1);

$data_documento = date('16/01/2017');
...
<?php
$query = "SELECT *, COUNT(*) as tot, SUM(commissione) as tot_commissione, SUM(imponibile) as tot_imponibile FROM fl_commissioni WHERE proprietario = $proprietario_id AND status_fatturazione = 1 AND data_operazione BETWEEN '$periodo_inizio' AND '$periodo_fine' GROUP BY prodotto_ref ";
...
<p><?php echo converti_txt($informazioni_cliente); ?></p>
  ...
  if(($dati['fattura_importi_zero'] == 0 && $totale_fattura > 0) || $dati['fattura_importi_zero'] == 1)
	...
	try
    {
		...
		$numero_documento++;
		echo $esito = '<p>'.$folder_fatture.$filename.' ultimo file creato!</p>';
		...
		$esito = "CREDITO NON INSERITO - TESTING ATTIVO";//insert_credito($proprietario_id,$periodo_inizio,$periodo_fine,$id_fattura,$numero_documento,$data_documento,$totale_commissioni,$_SESSION['number']);
	...
	//if(!mysql_query("UPDATE fl_config SET `ultimo_periodo_inizio` = '$periodo_inizio',`ultimo_periodo_fine` = '$periodo_fine' WHERE id = 2",CONNECT)) echo mysql_error();
	mysql_close(CONNECT);
