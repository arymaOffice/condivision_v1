...
if (isset($_POST['dir_upfile']) && @$_POST['dir_upfile'] != "") {
    $dir_upfile = $images . $_POST['dir_upfile'] . "/";
}

$sezione = "";
if (isset($_POST['relation'])) {
    $sezione = "&sezione=" . check($_POST['relation']);
}

if (isset($_POST['item_rel'])) {
    $sezione = "&item_rel=" . check($_POST['item_rel']);
}

if (isset($_GET['sezione'])) {
    $sezione = "&sezione=" . check($_GET['sezione']);
}

if (isset($_GET['tipo_segnalazione'])) {
    $sezione = "&tipo_segnalazione=" . check($_GET['tipo_segnalazione']);
}

if (isset($_POST['mode'])) {
    $sezione .= "&mode=" . check($_POST['mode']);
}
...
function not_doing($who)
{
    ...
    if (!is_numeric($who) && !in_array($who, $not_in)) {
        return true;
    }
...
if (isset($_POST['id'])) {
    ...
    if ($id == 1) {$id = new_inserimento($tabella, 0);
    ...
    foreach ($_POST as $chiave => $valore) {
        ...
        if (not_doing($chiave)) {
            ...
            if (isset($_POST['mandatory']) && $chiave != 'note' && (strstr(@$_POST['mandatory'], $chiave) || @strtolower($_POST['mandatory']) == "all")) {
                array_push($obbligatorio, $chiave);
            }
...
exit;
            ...
            if (in_array($chiave, $campi_date) && strpos($valore, '/') !== false) { // Inserimento date
                $data = explode("/", $valore);
...
if (@mysql_query($query, CONNECT)) {
    ...
    if (trim(check(@$_POST['old_file'])) != "") {
        $old_file = check(@$_POST['old_file']);
    }
