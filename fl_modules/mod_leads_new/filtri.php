...
<h1><strong><?php echo  mk_count('fl_potentials AS tb1 LEFT JOIN fl_veicoli AS tb2 ON tb1.id = tb2.parent_id  ',$where_count); ?></strong> leads in lista
  <?php if(isset($status_potential_id )) echo $status_potential[$status_potential_id]  ; ?>
  <?php 
...
<?php } else { ?>
<h1><strong><?php echo mk_count('fl_potentials AS tb1 LEFT JOIN fl_veicoli AS tb2 ON tb1.id = tb2.parent_id  ',$where_count); ?></strong> leads 
  <?php 
  if(isset($_GET['source_potential'])) { 
  foreach ($_GET['source_potential'] as $key => $value) {
  	echo $source_potential[check($value)].' '  ;
   ...
   <?php 
	...
	{		
			...
			if((select_type($chiave) == 2 || select_type($chiave) == 19 || select_type($chiave) == 9 || select_type($chiave) == 8 || select_type($chiave) == 12) && $chiave != 'id') {
				...
				echo '<div class="multicheckboxDiv"><label style="color: black;"><input type="checkbox" name="'.$chiave.'[]" value="-1"/>Non impostato</label>';
				foreach($$chiave as $val => $label) { 
				$selected = (isset($_GET[$chiave]) && is_array($_GET[$chiave]) && in_array($val,$_GET[$chiave])) ? 'checked' : ''; 
				echo '<label><input type="checkbox" '.$selected.' name="'.$chiave.'[]" value="'.$val.'" />'.$label.' </label>'; }
				echo '</div>';
				echo '</div>';
			} else if( $chiave != 'id') { 
...
select option:checked {
	...
	{		
			if(in_array($chiave,$basic_filters2)){
			...
			$cont = (isset($_GET[$chiave])) ? check($_GET[$chiave]) : ''; 
			echo '<div class="col-sm-2" style="display: inline-block; float:  none;">';
			echo '<label>'.$valore.'</label><input type="text" name="'.$chiave.'" value="'.$cont.'" />'; echo '</div>';
			}
