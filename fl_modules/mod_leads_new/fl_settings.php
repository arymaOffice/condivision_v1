<?php 
	...
	if(isset($_GET['id']) && check(@$_GET['id']) != 1) $tab_div_labels = array('mod_richieste.php?reQiD=[*ID*]'=>'Gestione BDC','id'=>"Dati Personali",'../mod_preventivi/mod_user.php?potential_id=[*ID*]'=>"Preventivi",'../mod_appuntamenti/mod_user.php?history&potential_rel=[*ID*]'=>"Agenda Incontri", 'tipo_interesse'=>"Interesse Vettura");
	...
	$basic_filters2 = array('alimentazione','anno_immatricolazione','pagamento_veicolo');
	$ordine_mod = array("id DESC"); // Tipologie di ordinamento disponobili 
	$ordine = (isset($_SESSION['ordine_type'])) ? ' tb1.id '.$_SESSION['ordine_type'] : 'tb1.id DESC';	
    ...
    if(!isset($_GET['all']) && $_SESSION['usertype'] != 0 && $_SESSION['profilo_funzione'] != 8 && $_SESSION['usertype'] != 5){ 
	...
	if(isset($_GET['todayCreated']))  $tipologia_main .= " AND DATE(tb1.`data_creazione`) = CURDATE()";
    ...
    if(isset($_GET['gestioneBdc']))  $tipologia_main .= "  AND proprietario > 0 AND venditore < 1";
	...
	$proprie = ($_SESSION['usertype'] > 2 && (!isset($_GET['status_potential']) || @$_GET['status_potential'] != 0))  ?  " AND proprietario = ".$_SESSION['number']." " : ''; // Aggiunta per i counters e il title counter
	...
	$invisible = array('data_assegnazione','data_scadenza_venditore','venditore','vettura_posseduta_marca', 'vettura_posseduta_modello', 'vettura_posseduta_immatricolazione', 'vettura_posseduta_km', 'vettura_posseduta_alimentazione', 'vettura_posseduta_targa', 'quotazione_vettura', 'data_acquisto_vettura',"website","fatturato_annuo","mansione","company","job_title","numero_dipendenti","data_creazione","sent_mail","in_use","status_potential",'is_customer','proprietario',"marchio","ip","operatore");
	$datePicker = array('data_scadenza');
	if(isset($_GET['id'])) { if(check($_GET['id']) > 1) $invisible[] = 'priorita_contatto'; $invisible[] = "note";  }
 ...
 foreach($status_potential as $valores => $label){ // Recursione Indici di Categoria
			...
			if($valores != 9 || $_SESSION['number'] == 1) $module_menu .= "<li $selected><a href=\"./?status_potential=$valores$action\">".ucfirst($label)."  </a></li>\r\n"; 
	}
