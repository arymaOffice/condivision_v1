...
$tipo_commensale  = $data_set->data_retriever('fl_commensali_tipo','plurale','WHERE 1');
...
<div class="elaborathio" style="font-size: 7pt; margin: 0; width: 760px; padding: 0; text-align: justify;">
...
<?php
	...
	$query = "SELECT persone.*, tavolo.* FROM `fl_tavoli_commensali` AS persone LEFT JOIN fl_tavoli AS tavolo ON persone.tavolo_id = tavolo.id WHERE tavolo.`evento_id` = '$evento_id' ORDER BY persone.tipo_commensale ASC, persone.`cognome` ASC;";
	
	$risultato = mysql_query($query, CONNECT);

	
	echo $tableHead = '
	<table class="dati" style="font-size: 11px;">
		<tr>
  		<th width="470">Cognome e nome</th>
  		<th>Lato</th>
  		<th>Tavolo</th>
  		<th>Adulti</th>
  		<th>Bambini</th>
	...
	if(mysql_affected_rows() == 0) { echo "<tr><td colspan=\"7\">Nessun ospite</td></tr>";		}
	
	while ($riga = mysql_fetch_array($risultato)) 
	{					
			

			if($riga['tipo_commensale'] > 3 && $riga['tipo_commensale'] != $tipoCommenCurrent) {

				$tipo_commensaleText = $tipo_commensale[$riga['tipo_commensale']];

				echo '<tr><td width="420"></td></tr><tr><td width="420">'.$tipo_commensaleText.'</td></tr>';
				$tipoCommenCurrent = $riga['tipo_commensale'];
			}
