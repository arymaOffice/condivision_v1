...
if(isset($_POST['leads'])) {
	...
	echo json_encode(array('action'=>'goto','class'=>'green','url'=>'./?status_potential='.$status,'esito'=>"Contatti in stato ".$status_potential[$status])); 
	mysql_close(CONNECT);
	exit;
	break;
...
if(isset($_POST['leads'])) {
	...
	$_SESSION['NOTIFY'] = "Contatti in stato: ".@$status_potential[$status];
	echo json_encode(array('action'=>'goto','class'=>'green','url'=>'./','esito'=>"Contatti Eliminati")); 
	mysql_close(CONNECT);
	exit;
	break;
