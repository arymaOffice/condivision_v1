...
$query = "SELECT $select,acc.id as accID ,a.id as anaID FROM `$tabella` a LEFT JOIN fl_account acc ON acc.anagrafica = a.id
	WHERE a.id >2 ORDER BY $ordine LIMIT $start,$step;";
...
while ($riga = mysql_fetch_array($risultato)) {
    ...
    if (ATTIVA_ACCOUNT_ANAGRAFICA == 1 && @$riga['accID'] > 0) {
        $user_check = '<a data-fancybox-type="iframe" title="Modifica Account" class="fancybox" href="../mod_account2/mod_visualizza.php?external&id=' . $riga['accID'] . '">' . $riga['user'] . '</a><br>' . $riga['motivo_sospensione'];
        $user_ball = ($riga['status_anagrafica'] == 1) ? "<span class=\"c-green\"><i class=\"fa fa-user\"></i></span>" : "<span class=\"c-red\"><i class=\"fa fa-user\"></i></span>";
        $saldo = balance($riga['accID']);
        $saldo = '<a data-fancybox-type="iframe" class="fancybox_view"  href="../mod_depositi/mod_user.php?operatore_text=' . $riga['nominativo'] . '&operatore=' . $riga['accID'] . '"> &euro; ' . numdec($saldo, 2) . '</a>';
        $tipo_profilo = $tipo[$riga['tipo']];
        $count++;
        $sendButton = '<input onClick="countFields(1);" type="checkbox" name="d[]" id="destinatario' . $riga['accID'] . '" value="' . $riga['accID'] . '" checked><label for="destinatario' . $riga['accID'] . '">' . $checkRadioLabel . '</label>';
        $notifica_icon = '<a data-fancybox-type="iframe" title="Invia Notifica Account" class="fancybox_view_small" href="../mod_notifiche/mod_invia.php?destinatario[]=' . $riga['accID'] . '"><i class="fa fa-bell" aria-hidden="true"></i></a>';
    } else {
        $user_check = "<a href=\"../mod_account2/mod_inserisci.php?external&anagrafica_id=" . $riga['anaID'] . "&email=" . $riga['email'] . "&nominativo=" . $riga['ragione_sociale'] . "\">Crea account</a>";
        $user_ball = '';
        $saldo = 0;
        $tipo_profilo = '';
        $notifica_icon = '';
        ...
        if ($riga['attivo'] == 1 && $riga['status_anagrafica'] == 1) {
            $colore = "b_green";
        } else if ($riga['attivo'] == 0 || $riga['status_anagrafica'] == 0) {
            $colore = "b_red";
        } else {
            $colore = "b_orange";
        }
