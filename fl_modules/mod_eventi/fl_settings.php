<?php 
	...
	if(defined('MULTI_LOCATION')) {
		if(!isset($_GET['location_evento']) && isset($_SESSION['sede_id'])) { 
			$_GET['location_evento'] = $_SESSION['sede_id']; $tipologia_main .= " AND location_evento = ".$_SESSION['sede_id']; 
			 } else if(!isset($_GET['location_evento'])) { $_GET['location_evento'] = DEFAULT_MEETING_LOCATION; }
	}


	$basic_filters = array('location_evento','ambiente_principale','centro_di_ricavo','ambienti','periodo_evento','stato_evento');
    ...
    $tema_evento = array('Personalizato','Classico','Moderno','Shabby Chic/Provenzale','Etnico','Altro');
    $tavolo_sposi_sedute = array('Troni','Sedie Uovo','Altro');
    $tavolo_sposi_allestimento = array('Tavolo Tradizionale','Tavolo Osvaldo');
	...
	$checkbox = array('tavolo_sposi_sedute','tavolo_sposi_allestimento','disposizione_tavoli','stato_evento','fiori','gruppo_musicale');
	$file = array();
	$timer = array();
	$touch = array('disposizione_tavoli');
	...
	*/
	...
	if(isset($_GET['tipo_evento']) && $_GET['tipo_evento'] != 9  && $_GET['tipo_evento'] != 5) array_push($hidden,'ora_del_rito','citta','wedding_planner','recapiti_wedding_planner','foto_e_auguri','fotografo','recapiti_fotografo','bomboniere','ospiti_serali','segnaposti','dolci');
	...
	$menu = array('Agenda'=>'../mod_eventi/?','Planning'=>'../mod_eventi/?action=24&closed','Definizione  Menù'=>'../mod_eventi/?action=24&closed&definizioneMenu','Conteggio Ospiti'=>'../mod_eventi/?action=24&closed&totaleOspiti');
	...
	if(isset($_GET['location_evento']))  $module_title .= ' '.@$location_evento[check($_GET['location_evento'])];
	if(isset($_GET['definizioneMenu']))  $module_title .= ' - Definizione  Menù';
	if(isset($_GET['totaleOspiti']))  $module_title .= ' - Conteggio degli ospiti';
	if(isset($_GET['Pagamenti']))  $module_title = ' BETA - Pagamenti';
