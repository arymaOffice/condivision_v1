...
if (GlobalToken != null && GlobalToken != '') {
  var userClean = localStorage.user.replace('"','');
  var userClean = userClean.replace('"','');
  $$('#nomeuser').html(userClean);
  $$('#form-lead')[0].reset();
  retrieve_data();
...
$$('#my-permuta').on('popup:close', function (e, popup) {
  ...
  if (modello_retrieve != '' && old_modello != $$('input[name="modello"]').val()) {
    ...
    $$('#permuta-button').html('Permuta inserita');
  }
  if (modello_retrieve == '') {
    $$('#permuta-button').html('Permuta non inserita');
      ...
      if (data.esito == 1) {
        app.request.json(endpoint, { usr_login: 1, user: username, password: password, token: data.token }, function (response) {
          if (response.esito == 1) {
            ...
            localStorage.setItem('id',window.usr_id);
    ...
    app.request({
      ...
      success: function (data) {
        ...
        if (data.class == 'green') {
          ...
          app.dialog.alert('Nuovo Lead Inserito, completare l\'operazione apponendo la firma nella schermata successiva.',function(){
