...
mail('fersinoandrea@gmail.com','Lancio servizio fatturazione GOS',count($affiliato).' Affiliati da prendere in analisi limite secondi: '.ini_get('max_execution_time'));
mail('support@aryma.it','Lancio servizio fatturazione GOS',count($affiliato).' Affiliati da prendere in analisi limite secondi: '.ini_get('max_execution_time'));	
$countFatture = 0;

foreach($affiliato as $chiave => $valore) {


$proprietario_id = $chiave;

$periodo_inizio = date("Y-m-1");
$periodo_fine = date('Y-m-15');


if(isset($_GET['fineMese'])) {
$periodo_inizio = date("Y-m-16", strtotime("previous month"));
$periodo_fine = date('Y-m-d', strtotime('last day of previous month'));
}


$data_documento = date('d/m/Y');
$numero_documento = get_next_fattura();
$informazioni_cliente = 'Fatturazione periodica commissioni';
...
<?php
$query = "SELECT *, COUNT(*) as tot, SUM(commissione) as tot_commissione, SUM(imponibile) as tot_imponibile FROM fl_commissioni WHERE proprietario = $proprietario_id AND data_operazione BETWEEN '$periodo_inizio' AND '$periodo_fine' GROUP BY prodotto_ref ";
...
<p><?php echo converti_txt($informazioni_cliente); ?></p>
  ...
  if(($dati['fattura_importi_zero'] == 0 && $totale_fattura > 0) || $dati['fattura_importi_zero'] == 1)
    {
	try
    {
        $content; 
      	$countFatture++;
      	set_time_limit (30); // ++ 30 secondi sul max execution time
		...
		} else { $esito = "Problema nella creazione della fattura - $query ".mysql_error(); }	
...
//echo '<span class="msg retrocolor">Elaborato in '. sprintf('%.2f',$callTime) , " secondi", " - Picco memoria: " , (memory_get_peak_usage(true) / 1024 / 1024) , " MB </span>" ; 


 mail('supporto@aryma.it',$countFatture.' emesse correttamente','Fattura emesse in '. sprintf('%.2f',$callTime).' secondi da  '.$_SERVER['REMOTE_ADDR']);
 mail('fersinoandrea@gmail.com',$countFatture.' emesse correttamente','Fattura emesse in '. sprintf('%.2f',$callTime).' secondi da  '.$_SERVER['REMOTE_ADDR']);
	...
	mysql_query("UPDATE fl_config SET `ultimo_periodo_inizio` = '$periodo_inizio',ultimo_periodo_fine = '$periodo_fine' WHERE id = 2",CONNECT);
	mysql_close(CONNECT);
