...
$insert = "insert into fl_esportazioni_excel (query,account_id) VALUES ('select  *,pot.id as poteID,ca.oggetto as caog FROM fl_potentials as pot LEFT JOIN fl_campagne_attivita ca ON ca.id = pot.campagna_id LEFT JOIN fl_veicoli v ON v.parent_id = pot.id WHERE ".$tipologia_main." ','".$_SESSION['number']."')";
mysql_query($insert);
mysql_close(CONNECT);
exit;
