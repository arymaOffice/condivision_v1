...
//if(!isset($_GET['label']) && $_SESSION['nome'] == 'Sistema') echo "<p><a href=\"./?action=1&sezione=&id=1\">Valori Predefiniti</a> | <a href=\"$thispage&label\">Modifica Campi</a> </p>";
...
if(isset($newid)) { $id = (is_numeric($newid)) ? check($newid) : exit; $where = "WHERE id = $id ";}	
	...
	echo "<input type=\"hidden\" name=\"gtx\" id=\"gtx\" value=\"$tab_id\" />";	
	
	if(isset($_GET['label'])) 
	{
	$x = 0; echo '</form><form method="post" enctype="application/x-www-form-urlencoded" action="../mod_basic/mod_save_labels.php" name="creazione_labels">';
	}
	
	$query = "SELECT * FROM `$tabella` $where LIMIT 1;";
	echo '<div id="results"></div>';

	if(!$risultato = mysql_query($query, CONNECT)) { echo "<p>".mysql_error()."</p><p>$query</p>";}
	...
	$campi = array();
	$campi_anno = array('anno_di_imposta');
	$campi_today = array('data_documento','data_pagamento','data_operazione','data_scadenza');

	while ($riga = mysql_fetch_array($risultato)) 
	{
		
		if($id > 1) action_record('open',$tabella,$id,base64_encode(json_encode($riga))); // 0 Record open 
		if(($_SESSION['usertype'] != 0 && $_SESSION['usertype'] != 4)  && isset($riga['identificato']) && @$riga['identificato'] > 1){ exit; }
	    $dafault_disable = ($_SESSION['usertype'] > 0 && @$riga['status_anagrafica'] > 1) ? "disabled" : "";
		...
		if(isset($tabs_div)) { if(in_array($chiave,$tabs_div) && !is_numeric($chiave)) echo '<p class=""><input type="submit" id="invio_scheda" value="Salva" class="button salva" /></p></div><div id="tb_'.$chiave.'">'; }	
		...
		if(is_to_use($chiave) == 1){
			...
			$tasto = '';
			$onblur = '';
			if($chiave == "partita_iva"){ $extra = '<a href="https://telematici.agenziaentrate.gov.it/VerificaPIVA/Scegli.do?parameter=verificaPiva" target="new"><img src="../../fl_set/img/verifica.png" alt="Verifica" class="info_row" /></a>'; }
		    if($chiave == "codice_fiscale" || $chiave == "codice_fiscale_legale"){ $extra = '<a href="https://telematici.agenziaentrate.gov.it/VerificaCF/Scegli.do?parameter=verificaCf" target="new"><img src="../../fl_set/img/verifica.png" class="info_row" alt="Verifica" /></a>'; }
			...
			if($chiave == "cap_punto" && isset($ajax))  $extra =  " <a href=\"#\" onClick=\"fl_cap(); return false;\"><i class=\"fa fa-map-marker\"></i> Rileva posizione</a>";
			if($chiave == "cap_punto" && isset($ajax))  $onblur = "onblur=\"fl_cap();\"";
			
			if($chiave == "cap_sede" && isset($ajax))  $extra =  " <a href=\"#\" onClick=\"fl_verifica_address('sede_legale','comune_sede','cap_sede'); return false;\" ><i class=\"fa fa-map-marker\"></i> Verifica Indirizzo</a>";
			if($chiave == "cap_sede" && isset($ajax))  $onblur = "onblur=\"fl_verifica_address('sede_legale','comune_sede','cap_sede'); \"";

			if($chiave == "cap_residenza" && isset($ajax))  $extra =  " <a href=\"#\" onClick=\"fl_verifica_address('indirizzo_residenza','comune_residenza','cap_residenza'); return false;\"><i class=\"fa fa-map-marker\"></i> Verifica Indirizzo</a>";
			if($chiave == "cap_residenza" && isset($ajax))  $onblur = "onblur=\"fl_verifica_address('indirizzo_residenza','comune_residenza','cap_residenza'); \"";
			...
			if($chiave == "proprietario"){  @define('PROPRIETARIO',$valore);   }
			if($chiave == "status_assistenza" ){ @define('STATUS_ASSISTENZA',$valore);  }
				echo '</div>';			
			...
			case 3:      // Text Area
			echo '<div class="form_row_txt" id="box_'.$chiave.'">';

			if($chiave != "messaggio" || $chiave != "note") echo "<h3><label for=\"$chiave\">".record_label($chiave,CONNECT,1)."</label></h3>";
			echo "<p class=\"input_text\"><br /><textarea  name=\"$chiave\" id=\"$chiave\" rows=\"20\" cols=\"100\" $disabled>".$valore."</textarea></p>\r\n";
			echo '</div>';	
			break;
			...
			echo '<div class="form_row" id="box_'.$chiave.'">';
  		    ...
  		    echo "<p class=\"input_text\"><label for=\"$chiave\">".record_label($chiave,CONNECT,1)."</label><input type=\"text\" name=\"$chiave\" id=\"$chiave\"  value=\"".$valore."\" disabled /> </p>\r\n";
			echo '</div>';			
			break;
			...
			if($chiave == "proprietario" && $id == 1 && isset($_GET['proprietario'])){ $valore = check($_GET['proprietario']);}
			if($chiave == "id"){ $file_id = $valore; }
			echo "<input type=\"hidden\" name=\"$chiave\" id=\"$chiave\" value=\"$valore\" style=\"display: none;\"  />";
			break;
			
			case 6:
		    $cheking = ($valore == 1) ? "checked=\"checked\"" : "";
			echo "<p class=\"input_check\"><input type=\"checkbox\" name=\"$chiave\" id=\"$chiave\" value=\"1\" $cheking  $disabled /><label for=\"$chiave\">".record_label($chiave,CONNECT,1)."</label></p>\r\n";
			break;
			...
			$counter++;
			if($counter == 9 && $tab_id == 30){ echo "</td><td>";}		
		   	echo "</div><div>
			<label class=\"desc_label\">".record_label($chiave,CONNECT,1)."</label> <input type=\"radio\" id=\"".$chiave."1\" name=\"$chiave\" value=\"1\" $cheking />
			...
			if($valore != "") echo "<p><a href=\"$dir_testate$valore\" onclick=\"javascript:window.open(this.href); return false;\"><img src=\"$dir_testate$valore\" alt=\"Immagine Principale\" class=\"icona_catalogo\" /></a></p>";
			echo "<p class=\"input_text\"><label for=\"$chiave\">".record_label($chiave,CONNECT,1)."</label><input type=\"file\" name=\"$chiave\" id=\"$chiave\" size=\"10\" /></p>\r\n";
			if($valore != "") echo "<p>Carica per sovrascrivere o seleziona per eliminare: <input type=\"hidden\" name=\"old_file\" id=\"old_file\" value=\"$valore\" /><input type=\"checkbox\" name=\"del_file\" id=\"del_file\" value=\"1\" /> - [Elimina]</p>";
				echo '</div>';	
				break;
			...
			echo "<p class=\"select_text $chiave\" ><label for=\"$chiave\">".record_label($chiave,CONNECT,1)."</label>\r\n
			<select class=\"$obbligatorio\" name=\"$chiave\"  disabled>\r\n";
			echo "<option value=\"\">Seleziona...</option>\r\n";
			...
			<label for=\"".$chiave.$valores."\" style=\"max-width: 20%; margin: 0 5px 0 0; padding: 5px;\">".$label."</label>";
			}
			...
			case 23:
			echo "<p>".record_label($chiave,CONNECT,1)."";
			$valore = explode(',',$valore);
			foreach($$chiave as $valores => $label){ // Recursione Indici di Categoria
			$selected = ($valore[$valores] == 1) ? "checked=\"checked\"" : "";	
	
		   	echo "
			<input name=\"$chiave"."[".$valores."]"."\" type=\"checkbox\" id=\"".$chiave.$valores."\" value=\"".$valore[$valores]."\" $selected />
	...
	echo '<p class=""><input type="submit" id="invio_scheda" value="Salva" class="button salva" /></p></div>';
	foreach($tabs_ext as $chiave => $valore) { 
	 $contenutoFrame = str_replace('[*ID*]',$id,$valore);
     if(!is_numeric($chiave)) echo "<div id=\"$chiave\"><iframe style=\"width: 100%; border: none; height:1000px; \" src=\"".$contenutoFrame."\"></iframe></div>";
	}
    }
	
	if(isset($tabs_div) && @$tabs_div != 0) {  
	echo '</div>'; 
	} else { echo '<p class=""><input type="submit" id="invio_scheda" value="Salva" class="button salva" /></p>'; }
...
if(isset($newid)) { $id = (is_numeric($newid)) ? check($newid) : exit; $where = "WHERE id = $id ";}	
	...
	$campi_today = array('data_scadenza_contratto','start_meeting','end_meeting','data_fine_evento','data_evento','data_preventivo','data_pubblicazione','data_prenotazione','data_pagamento','data_richiesta','data_fattura','data_apertura','data_avvio','data_corrispettivo','data_versamento','data_scadenza_pec','data_sottoscrizione','data_inizio','data_fine','data_rinnovo','data_documento','data_operazione','data_scadenza');
	...
	while ($riga = mysql_fetch_array($risultato)) 
	{
		...
		if(is_to_use($chiave) == 1){
			...
			$disabilitati = array('pincode','aggiornameto_pin','aggiornamento_password','data_creazione','account_id','data_inizio','data_fine','datac_inizio','datac_fine','account_affiliato','anagrafica');		
		    if(in_array($chiave,$disabilitati)){ $disabled = "disabled";  }
